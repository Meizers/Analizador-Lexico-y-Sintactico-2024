...
// logica para identificar ciertos tipos de token
LETRA = [a-zA-Z]
DIGITO = [0-9]
ESPACIO = [ \t\f\n\r\n]+
...
%{
/*  aca creamos el metodo para actualizar la tabla de simbolo, esto en la 2da entrega hay que colocarlo en el sintactico.cup
...
// Lista de Tokens, En la primera entrega dentro de los corchetes hay que mostrar por la vista el token encontrado, en la 2da
// simplmenete retornamos el objeto sym y ponemos el nombre del token para que lo use el sintactico.cup
// el yyline y yycolum es para que pueda saber la linea y columna donde se encuentra el token para marcarlo en los errores
<YYINITIAL> {
...
{ID} {
    /*  Actualizacion de la tabla de simbolo al encontrar el token ID, esto para la segunda entrega hay que ponerlo en el Sintactico.cup
    String id = yytext();
...
// programa que se lanza al encontrar un error
